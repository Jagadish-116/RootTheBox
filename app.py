...
# In-memory store for active terminal sessions: session_id -> TerminalSession
sessions = {}
...
def get_or_create_session(scenario_id=1):
    ...
    # If session doesn't exist, or scenario has changed, initialize a new terminal state
...
def submit_flag():
    ...
    # We verify:
    # 1. Did the user actually escalate?
    # 2. Does the flag string match exactly?
    if not term_sess.escalated:
        # If not escalated, check if they somehow got the flag file content?
        # Standard flow requires getting admin shell first
        # But we can allow matching the flag directly too
        pass
...
if __name__ == '__main__':
    print("Starting Windows Privilege Escalation Lab server...")
